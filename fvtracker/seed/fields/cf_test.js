...
async function test() {
const promises = [];
  const parameters = generateParameters(5);
  for (let _fieldParams of generateParameters(5)) {
    promises.push(createField(_fieldParams, 5000));
  }

  const fields = await Promise.all(promises);
  for (let i = 0; i<parameters.length; i++) {
    console.log(`\n\nField ${i + 1} created with parameters:`);
    for (const [key, value] of Object.entries(parameters[i])) {
      console.log(`${key}: ${value}`);
    }
    fieldFilledRatio(fields[i]);
  }
...
function generateParameters(n) {
  ...
  for (let i = 0; i < n; i++) {
    const param = {
      ...
      min_ca_dim: Math.floor(Math.random() * 20) + 5,
      max_ca_dim: Math.floor(Math.random() * 30) + 20,
